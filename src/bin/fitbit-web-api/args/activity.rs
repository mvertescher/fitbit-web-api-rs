...
pub(super) fn app() -> App<'static, 'static> {
    App::new(BASE)
        ...
        .subcommand(SubCommand::with_name(BASE_LIFETIME_STATS)
                    .about("Print a summary of the user's lifetime activity statistics"))
        .subcommand(SubCommand::with_name(BASE_SUMMARY)
        ...
        .subcommand(SubCommand::with_name(BASE_TS)
                    ...
                    .setting(AppSettings::SubcommandRequiredElseHelp)
                    .subcommand(SubCommand::with_name(BASE_TS_CALORIES)
                                .about("Print calories time series data"))
                    .subcommand(SubCommand::with_name(BASE_TS_CALORIES_BMR)
                                .about("Print calories BMR time series data"))
                    .subcommand(SubCommand::with_name(BASE_TS_STEPS)
                                .about("Print steps time series data"))
                    .subcommand(SubCommand::with_name(BASE_TS_DISTANCE)
                                .about("Print distance time series data"))
                    .subcommand(SubCommand::with_name(BASE_TS_FLOORS)
                                .about("Print floors time series data"))
                    .subcommand(SubCommand::with_name(BASE_TS_ELEVATION)
                                .about("Print elevation time series data"))
                    .subcommand(SubCommand::with_name(BASE_TS_SEDENTARY)
                                .about("Print minutes sedentary time series data"))
                    .subcommand(SubCommand::with_name(BASE_TS_LIGHTLY_ACTIVE)
                                .about("Print lightly active time series data"))
                    .subcommand(SubCommand::with_name(BASE_TS_FAIRLY_ACTIVE)
                                .about("Print fairly active time series data"))
                    .subcommand(SubCommand::with_name(BASE_TS_VERY_ACTIVE)
                                .about("Print very active time series data"))
                    .subcommand(SubCommand::with_name(BASE_TS_ACTIVITY_CALORIES)
    ...
    match matches.subcommand() {
        ...
        (BASE_TS, Some(activity_ts_matches)) => {
            ...
            match activity_ts_matches.subcommand() {
                ...
                (BASE_TS_CALORIES_BMR, Some(_)) => Command::GetActivityTimeSeries(Resource::CaloriesBMR),
                ...
                (BASE_TS_LIGHTLY_ACTIVE, Some(_)) => Command::GetActivityTimeSeries(Resource::LightlyActive),
                (BASE_TS_FAIRLY_ACTIVE, Some(_)) => Command::GetActivityTimeSeries(Resource::FairlyActive),
                (BASE_TS_VERY_ACTIVE, Some(_)) => Command::GetActivityTimeSeries(Resource::VeryActive),
                (BASE_TS_ACTIVITY_CALORIES, Some(_)) => Command::GetActivityTimeSeries(Resource::ActivityCalories),
